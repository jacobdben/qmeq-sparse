...
cdef class ApproachPauli(Approach):
    ...
    cdef void prepare_arrays(self):
        ...
        ################################################################################################################
        self.paulifct = np.zeros((nleads, ndm1, 4), dtype=doublenp)
        ################################################################################################################
    ...
    cpdef void generate_fct(self):
        ...
        for i in range(kh.ndm1):
            ...
            for l in range(nleads):
                ...
                func_pauli(-Ecb, mulst[l], tlst[l], dlst[l, 0], dlst[l, 1], itype, rez)
                paulifct[l, cb, 2] = xbc * rez[0]
                paulifct[l, cb, 3] = xbc * rez[1]
                ########################################################################################################
    ...
    cdef void generate_coupling_terms(self,
        ...
        for i in range(acount):
            ...
            for l in range(nleads):
                ########################################################################################################
                fctm -= paulifct[l, ba, 1] + paulifct[l, ba, 2]
                fctp += paulifct[l, ba, 0] + paulifct[l, ba, 3]
                ########################################################################################################
        ...
        for i in range(ccount):
            ...
            for l in range(nleads):
                ########################################################################################################
                fctm -= paulifct[l, cb, 0] + paulifct[l, cb, 3]
                fctp += paulifct[l, cb, 1] + paulifct[l, cb, 2]
                ########################################################################################################
    ...
    cpdef void generate_current(self):
        ...
        cdef double_t fct1, fct2, fct3, fct4
        ...
        for i in range(kh.ndm1):
            ...
            for l in range(nleads):
                ...
                current[l] = current[l] + fct1 + fct2 + fct3 + fct4
                energy_current[l] = energy_current[l] - (E[b]-E[c])*(fct1 + fct2 + fct3 + fct4)
                ########################################################################################################
